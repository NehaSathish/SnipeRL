# -*- coding: utf-8 -*-
"""
Created on Mon Apr 26 19:50:00 2021
...
'''
# Before running this script , create a file called log.txt and write the date
 from which you want to download EOD data
...
for i in range(1, diff.days + 1):
    ...
    if a.status_code == 200:
        ...
        os.remove(zpath)
        f, deldict = (
            pd.read_csv(base + y + "/cm" + d + dmonth[m] + y + "bhav.csv"),
            {},
        )  # reading the raw dl-ed bhav file
        f = f[
            f["SERIES"] == "EQ"
        ]  # retaining only EQ rows and leaving out bonds,options etc
        deliverable = requests.get(
            "https://nseindia.com/archives/equities/mto/MTO_" + d + m + y + ".DAT"
        ).text.splitlines()
        del deliverable[:4]
        ...
        indices = requests.get(
            "https://nseindia.com/content/indices/ind_close_all_" + d + m + y + ".csv"
        ).content

        # sometimes nse doesnt give the index file, so the if condition
        if len(indices) > 300:
            indx = pd.read_csv(
                io.StringIO(indices.decode("utf-8"))
            )  # reading content of indices csv and storing in DataFrame using io module
            indx.to_csv(
                base + y + "/Index/Indices" + str(nextdt.date()) + ".csv", index=False
            )
            indx[
                [
                    "Index Name",
        ...
        f["TIMESTAMP"] = pd.Series(
            str(nextdt.date().strftime("%Y%m%d")) for _ in range(len(f))
        )
        f = f[
            [
                "SYMBOL",
        ...
        futures = requests.get(
            "https://nseindia.com/content/historical/DERIVATIVES/"
            + y
            + "/"
            + dmonth[m]
            + "/fo"
